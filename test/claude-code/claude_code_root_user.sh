...
if [ -d "$ROOT_HOME" ]; then
    ...
    check ".claude points to persistent volume" bash -c "[ \"\$(readlink -f $ROOT_HOME/.claude)\" = \"/var/lib/claude-config\" ]"

    # Check ownership - should be 'root' user
    check ".claude symlink owned by root" bash -c "stat -c '%U' $ROOT_HOME/.claude | grep 'root'"
    check "persistent volume owned by root" bash -c "stat -c '%U' /var/lib/claude-config | grep 'root'"
    ...
    check "root can write to persistent volume" bash -c "touch /var/lib/claude-config/test-root-file && rm /var/lib/claude-config/test-root-file"
