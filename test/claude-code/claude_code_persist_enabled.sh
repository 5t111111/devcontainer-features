...
check "persistent xdg directory exists" test -d "/var/lib/claude-config/xdg"
...
if [ -d "$USER_HOME" ]; then
    ...
    check "persistent volume ownership" bash -c "stat -c '%U' /var/lib/claude-config/home | grep -E 'vscode|root'"
    ...
    check "can create file in persistent volume" bash -c "touch /var/lib/claude-config/home/.test-write && rm /var/lib/claude-config/home/.test-write"
    ...
    check "persistent home volume is a directory" test -d "/var/lib/claude-config/home"
