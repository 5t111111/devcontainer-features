...
if [ -d "$USER_HOME" ]; then
    ...
    check ".claude.json points to correct location" bash -c "[ \"\$(readlink $USER_HOME/.claude.json)\" = \"/var/lib/claude-config/home/config.json\" ]"

    # Verify config.json exists in persistent volume
    check "config.json exists in persistent volume" test -f "/var/lib/claude-config/home/config.json"

    # Verify config.json is valid JSON (at minimum an empty object)
    check "config.json is valid JSON" bash -c "cat /var/lib/claude-config/home/config.json | grep -q '{'"

    # Verify we can read through the symlink
    check "can read .claude.json via symlink" test -r "$USER_HOME/.claude.json"

    # Verify the file is accessible
    check "config.json content is readable" cat /var/lib/claude-config/home/config.json >/dev/null 2>&1

    # Verify ownership
    check ".claude.json symlink ownership" bash -c "stat -c '%U' $USER_HOME/.claude.json | grep -E 'vscode|root'"
    check "config.json ownership" bash -c "stat -c '%U' /var/lib/claude-config/home/config.json | grep -E 'vscode|root'"
fi
