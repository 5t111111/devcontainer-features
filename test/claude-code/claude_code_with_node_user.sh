...
if [ -d "$NODE_HOME" ]; then
    ...
    check ".claude points to persistent volume" bash -c "[ \"\$(readlink -f $NODE_HOME/.claude)\" = \"/var/lib/claude-config/home\" ]"

    # Check ownership - should be 'node' user
    check ".claude symlink owned by node" bash -c "stat -c '%U' $NODE_HOME/.claude | grep 'node'"
    check "persistent volume owned by node" bash -c "stat -c '%U' /var/lib/claude-config/home | grep 'node'"

    # Verify can write to persistent volume (as root during test)
    check "can write to persistent volume" bash -c "touch /var/lib/claude-config/home/.test-file && rm /var/lib/claude-config/home/.test-file"
else
